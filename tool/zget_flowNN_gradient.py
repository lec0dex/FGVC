...
def zget_flowNN_gradient(args,
    ...
    # videoFlow:          imgH x imgW x 2 x 2 x (nFrame - 1) | [u, v] | [f, b]
    # videoNonLocalFlowF: imgH x imgW x 2 x 3 x nFrame
    # videoNonLocalFlowB: imgH x imgW x 2 x 3 x nFrame
    gradient_x = zgradient_x[:, :, :, rFrames]
    gradient_y = zgradient_y[:, :, :, rFrames]
    mask = zmask[:, :, rFrames]
    #indFrameSet = np.arange(rFrames.start, rFrames.stop, rFrames.step)
    indFrameSet = range(rFrames.start or 0, rFrames.stop or len(mask), rFrames.step or 1)
    ...
    for indFrame in frameIndSetF:
        ...
        flowB_vertical = videoFlow[:, :, 1, 1, indFrameSet[indFrame - 1]]  # t --> t-1
        flowB_horizont = videoFlow[:, :, 0, 1, indFrameSet[indFrame - 1]]
        flowF_vertical = videoFlow[:, :, 1, 0, indFrameSet[indFrame - 1]]  # t-1 --> t
        flowF_horizont = videoFlow[:, :, 0, 0, indFrameSet[indFrame - 1]]
        ...
        BFdiff, BF_uv = consistCheck(videoFlow[:, :, :, 0, indFrameSet[indFrame - 1]],
                                     videoFlow[:, :, :, 1, indFrameSet[indFrame - 1]])
    ...
    for indFrame in frameIndSetB:
        ...
        flowF_vertical = videoFlow[:, :, 1, 0, indFrameSet[indFrame]]  # t --> t+1
        flowF_horizont = videoFlow[:, :, 0, 0, indFrameSet[indFrame]]
        flowB_vertical = videoFlow[:, :, 1, 1, indFrameSet[indFrame]]  # t+1 --> t
        flowB_horizont = videoFlow[:, :, 0, 1, indFrameSet[indFrame]]
        ...
        FBdiff, FB_uv = consistCheck(videoFlow[:, :, :, 1, indFrameSet[indFrame]],
                                     videoFlow[:, :, :, 0, indFrameSet[indFrame]])
    ...
    for indFrame in range(nFrame):
        ...
        print("{0:8d} pixels are from source Frame {1:3d}"
                        .format(len(SourceFmInd[0]), indFrameSet[indFrame]))
    ...
    for indFrame in range(nFrame):
        ...
        zgradient_x[:, :, :, indFrameSet[indFrame]] = gradient_x[:, :, :, indFrame]
        ...
        zgradient_y[:, :, :, indFrameSet[indFrame]] = gradient_y[:, :, :, indFrame]

        mask_tofill[np.logical_and(np.invert(HaveNN_sum), mask[:, :, indFrame]), indFrame] = True
        zmask[:, :, indFrameSet[indFrame]] = mask_tofill[:, :, indFrame]
